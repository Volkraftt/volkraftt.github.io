...
function conclick() {
  ...
  document.getElementById("con-inp-btn").style.transform = "translateY(-1%)";
    ...
    document.getElementById("con-slide").style.maxHeight = "1738px";
  document.getElementById("con-box").style.maxHeight = "950px";
  document.getElementById("con-box-ins").style.transform = "translateY(0%)";
  document.getElementById("con-inp-btn").style.transform = "translateY(-1%)";
  }
}
...
const element = document.getElementById('f-visit-close');

// Проверяем, есть ли в localStorage значение для ключа "isHidden"
if (localStorage.getItem('isHidden')) {
  // Если значение равно "true", скрываем элемент
...
element.addEventListener('i-slide-but-close', function() {
  // Скрываем элемент
  this.style.display = 'none';
  
  // Сохраняем значение "true" для ключа "isHidden" в localStorage
